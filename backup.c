...
{
    int left = (int)x;
    int right = (int)x + HITBOX_W;
    int top = (int)y;
    int bottom = (int)y + HITBOX_H;

    for (int py = top; py < bottom; py += 2)
    {
        for (int px = left; px < right; px += 2)
        {
            if (pixel_solido(mapa, px, py))
            {
                return true;
            }
        }
...
void desenhar_hitbox_mapa(ALLEGRO_BITMAP *mapa)
{
    static int desenhado = 0;

    if (desenhado)
        return;

    for (int y = 0; y < al_get_bitmap_height(mapa); y += 16)
    {
        for (int x = 0; x < al_get_bitmap_width(mapa); x += 16)
        {
            if (pixel_solido(mapa, x, y))
            {
    ...
    while (rodando)
    {
        ...
        if (ev.type == ALLEGRO_EVENT_TIMER)
        {
            ...
            al_draw_rectangle(
                jogador.mov.x,
                jogador.mov.y,
                jogador.mov.x + HITBOX_W,
                jogador.mov.y + HITBOX_H,
                al_map_rgb(255, 0, 0),
                2);
