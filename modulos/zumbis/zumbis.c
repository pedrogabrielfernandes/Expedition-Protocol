                                ...
                                ALLEGRO_BITMAP *mapa)
{
    ...
    for (int i = 0; i < MAX_ZUMBIS_TELA; i++)
    {
        ...
        if (z->estado == ZUM_ATTACK || z->estado == ZUM_BITE)
        {
            ...
            int max_f = (z->estado == ZUM_ATTACK) ? FRAMES_ZUMBI_ATTACK1
                                                   : FRAMES_ZUMBI_BITE;
...
void horda_verificar_ataque(Horda *h, Jogador *jog, Sanidade *san, Sons *sons)
{
    ...
    float offset = 10.0f;
    float atk_x = (jog->direcao == 0) ? jog_cx + offset
                                       : jog_cx - atk_w - offset;
    ...
    for (int i = 0; i < MAX_ZUMBIS_TELA; i++)
    {
            ...
            atk_y < hzy + hzh && atk_y + atk_h > hzy)
        {
            ...
            int dano = (jog->tipo_ataque == 3) ? 2
                     : (jog->tipo_ataque == 2)  ? jog->dano_atk2_atual
                                                : 1;
            z->vida -= dano;
            if (z->vida < 0)
                z->vida = 0;
                                   ...
                                   VidaStatus *vidas, Sons *sons)
{
    ...
    for (int i = 0; i < MAX_ZUMBIS_TELA; i++)
    {
        ...
        float atk_x = (body_centro < jog_centro) ? body_x + ZUMBI_HBX_W
                                                  : body_x - atk_w;
        float atk_y = body_y;
                    ...
                    ZumbiAcidoSprites *spr_acido)
{
    for (int i = 0; i < MAX_ZUMBIS_TELA; i++)
    {
        ...
        if (z->tipo == 2)
        {
            switch (z->estado)
            {
            case ZUM_WALK:   sheet = spr_acido->walk;   max_f = FRAMES_ACIDO_WALK;   break;
            case ZUM_ATTACK: sheet = spr_acido->attack; max_f = FRAMES_ACIDO_ATTACK; break;
            case ZUM_HURT:   sheet = spr_acido->hurt;   max_f = FRAMES_ACIDO_HURT;   break;
            case ZUM_DEAD:   sheet = spr_acido->dead;   max_f = FRAMES_ACIDO_DEAD;   break;
            default:         sheet = spr_acido->idle;   max_f = FRAMES_ACIDO_IDLE;   break;
            }
        }
        else
        {
        ...
        if (z->estado != ZUM_DEAD)
        {
            ...
            al_draw_filled_rectangle(bx2, by2, bx2 + bw2, by2 + 6,
                                     al_map_rgb(80, 0, 0));
            al_draw_filled_rectangle(bx2, by2, bx2 + bw2 * vida_pct, by2 + 6,
                                     al_map_rgb(255, 0, 0));
        }
    }
}
