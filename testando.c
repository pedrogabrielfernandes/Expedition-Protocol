...
#define SPRITE_SRC_W 450
#define SPRITE_SRC_H 280
...
int main(void)
{
    ...
    if (!bg_menu)
    {
        printf("Erro ao carregar background menu\n");
        return 1;
    }
    ...
    ALLEGRO_BITMAP *mapa =
        al_load_bitmap("assets/cenarios/colisao1.png");
    ...
    while (rodando)
    {
        ...
        if (ev.type == ALLEGRO_EVENT_TIMER)
        {
            ...
            if (
                al_key_down(&state, ALLEGRO_KEY_W) &&
                jogador.no_chao
            )
            {
