...
void horda_atualizar_movimento(Horda *h, Jogador *j, Sons *sons, ALLEGRO_BITMAP *mapa, const ConfigMapa *cfg);
...
static int mouse_sobre_slider(float mx, float my, float sx, float sy)
{
    ...
    return (mx >= sx && mx <= sx + SLIDER_LARGURA &&
...
/* ================================================================== */
static void carregar_cenario(const ConfigMapa *cfg,
                             ALLEGRO_BITMAP **bg_out,
...
static void preparar_nova_fase(Jogador *jogador, Horda *horda, Pocao *pocao,
                               double *pocao_ultimo_check,
                               Temporizador *tempo, const ConfigMapa *cfg)
{
    ...
    while (rodando_global)
    {
        ...
        if (fase_escolhida == 0)
        {
            ...
            while (em_cutscene)
            {
                ...
                if (ev_cut.type == ALLEGRO_EVENT_MOUSE_BUTTON_DOWN && ev_cut.mouse.button == 1)
        ...
        while (rodando)
        {
            ...
            if (ev.type == ALLEGRO_EVENT_MOUSE_BUTTON_DOWN && ev.mouse.button == 1)
            {
                ...
                if (pausado)
                {
                    ...
                    else if (mouse_sobre_slider(mx, my, SLIDER_MUSICA_X, SLIDER_MUSICA_Y))
                    {
            ...
            if (!pausado && !em_game_over && !horda.fase_concluida)
            {
                ...
                if (jogador.dash_fuga_ativo)
                {
                    ...
                    {
                        float move = (jogador.dash_fuga_dist < passo_fuga)
                                         ? jogador.dash_fuga_dist
                                         : passo_fuga;
                ...
                if (boss_ativo_nessa_fase && boss.ativo)
                {
                    ...
                    if (jogador.atacando && !jogador.dash_fuga_ativo)
                    {
                        ...
                        if (bx1 < jx2 && bx2 > jx1 &&
                            by1 < jy2 && by2 > jy1)
                        {
                            int dano_sam = (jogador.tipo_ataque == 2)
                                               ? jogador.dano_atk2_atual
                                           : (jogador.tipo_ataque == 3) ? DANO_ATK3
                                                                        : DANO_ATK1;
            ...
            /* Indicador de fase atual no canto superior direito */
            {
                ...
                al_draw_text(fonte_hud,
                             al_map_rgba(220, 180, 60, 200),
                             LARGURA - 200, 47,
                             ALLEGRO_ALIGN_RIGHT, txt_fase);
            }
            ...
            if (morte_pronta || sanidade.game_over)
            {
                ...
                int reiniciar = tela_game_over(queue, timer, fonte, fonte_hud, motivo, &sons);
                if (reiniciar)
                {
