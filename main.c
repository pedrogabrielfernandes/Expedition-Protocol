...
{
    if (!s->padrao || s->padrao_tocando) return;
    ...
    while (1)
    {
        ...
        if (ev.type == ALLEGRO_EVENT_KEY_DOWN)
        {
            ...
            if (ev.keyboard.keycode == ALLEGRO_KEY_D || ev.keyboard.keycode == ALLEGRO_KEY_RIGHT)
                opcao = MENU_SAIR;
            if (opcao != antes) tocar(sons->selecao_som);
            if (ev.keyboard.keycode == ALLEGRO_KEY_ENTER || ev.keyboard.keycode == ALLEGRO_KEY_SPACE)
            {
                if (opcao == MENU_JOGAR) tocar(sons->jogar);
                else                     tocar(sons->sair_som);
                al_rest(0.3);
                return opcao;
            }
        }
...
{
    (void)mapa;
    if (x < 0 || y < 0 || x >= 3000 || y >= ALTURA) return true;
    return colisao[y][x];
}
...
void atualizar_sanidade(Sanidade *san)
{
    ...
    if (san->valor > alvo) san->valor = alvo;
    if (san->valor <= 0.0f)
    {
        san->valor    = 0.0f;
        san->game_over = 1;
    ...
    if (sem_matar >= TEMPO_INICIO_RECUPERACAO && !san->game_over)
    {
        ...
        if (novo_valor > san->valor) san->valor = novo_valor;
...
{
    if (zumbis_no_ciclo(san) < AVISO_ZUMBIS || san->game_over) return;
...
void pocao_tentar_spawn(Pocao *p, VidaStatus *vidas)
{
    if (p->ativa || contar_vidas(vidas) != 1) return;
...
void pocao_atualizar(Pocao *p, Jogador *jog, VidaStatus *vidas, Sons *sons)
{
    ...
    if (!p->no_chao)
    {
        ...
        {
            while (!pocao_piso_abaixo(jog_mapa_ptr, p->x, p->y + 1.0f)) p->y += 1.0f;
            p->vel_y  = 0.0f;
            p->no_chao = 1;
        }
        else p->y = ny;
...
{
    for (int i = 0; i < MAX_ZUMBIS_TELA; i++) h->pool[i].vivo = 0;
    h->total_spawned  = 0;
    h->total_mortos   = 0;
    h->timer_onda     = 180;
    h->fase_concluida = 0;
...
static void spawnar_zumbi(Horda *h, Sons *sons, int s, float sx_, float sy_, int spawn_tipo)
{
    Inimigo *z = &h->pool[s];
    z->x = sx_; z->y = sy_;
    z->vel_y = 0.0f; z->x_inicial = sx_; z->y_inicial = sy_;
    z->patrol_base = sx_; z->patrol_dir = (rand()%2==0) ? 1.0f : -1.0f;
    z->direcao = ALLEGRO_FLIP_HORIZONTAL; z->vivo = 1;
    z->estado = ZUM_WALK; z->frame = (float)(rand()%8);
    z->tempo_hurt = 0.0; z->atacando_jogador = 0;
    z->tempo_ataque = 0.0; z->spawn_tipo = spawn_tipo;
    z->dano_aplicado = 0; z->atingido_no_ataque = 0;
    z->ataque_som_tocado = 0; z->ataque_resultado = 0;
    z->ultimo_dano_recebido = 0.0;
    z->tempo_stun = 0.0; z->stunado = 0; z->ja_stunado_no_dash = 0;
...
void horda_atualizar_spawn(Horda *h, Sons *sons, float jogador_y)
{
    ...
    if (jogador_y < SPAWN_MIN_JOGADOR_Y) return;
    if (zumbis_vivos_na_tela(h) > 2) return;
    if (--h->timer_onda <= 0)
    {
...
void horda_atualizar_fisica(Horda *h, ALLEGRO_BITMAP *mapa)
{
    for (int i = 0; i < MAX_ZUMBIS_TELA; i++)
    {
        ...
        if (z->vel_y > 0.0f)
        {
            ...
            {
                while (!zumbi_colide_vertical(mapa, z->x, z->y+1.0f, 1.0f)) z->y += 1.0f;
                z->vel_y = 0.0f;
            }
            else z->y = ny;
        }
        ...
        {
            if (zumbi_colide_vertical(mapa, z->x, ny, z->vel_y)) z->vel_y = 0.0f;
            else z->y = ny;
        }
...
void horda_atualizar_movimento(Horda *h, Jogador *j, Sons *sons, ALLEGRO_BITMAP *mapa)
{
    ...
    for (int i = 0; i < MAX_ZUMBIS_TELA; i++)
    {
        ...
        if (z->stunado)
        {
            ...
            {
                z->stunado = 0; z->atacando_jogador = 0;
                z->dano_aplicado = 0; z->frame = 0; z->estado = ZUM_WALK;
            }
        }
        ...
        if (z->estado == ZUM_HURT)
        {
            ...
            {
                z->frame = 0; z->estado = ZUM_WALK;
                z->atacando_jogador = 0; z->dano_aplicado = 0;
            }
            continue;
        }
             ...
             !(fabsf(jog_cx-zumbi_cx)<250.0f && fabsf(jog_cy-zumbi_cy)<250.0f)))
        {
            if (agora - z->tempo_ataque > 1.5)
            {
                ...
                int r = rand()%3;
                z->patrol_dir = (r==0) ? -1.0f : (r==1) ? 1.0f : 0.0f;
            }
        ...
        if (dx < 55.0f && dy < 60.0f)
        {
            ...
            {
                z->atacando_jogador = 1; z->tempo_ataque = agora;
                z->frame = 0; z->dano_aplicado = 0;
                z->ataque_som_tocado = 0; z->ataque_resultado = 0;
                z->x += ((i%5)-2) * 6.0f;
        ...
        else
        {
            ...
            if (jog_cx < zumbi_cx)
            {
                ...
                if (!zumbi_colide_horizontal(mapa, nx_z, z->y)) z->x = nx_z;
...
void horda_verificar_ataque(Horda *h, Jogador *jog, Sanidade *san, Sons *sons)
{
    ...
    for (int i = 0; i < MAX_ZUMBIS_TELA; i++)
    {
        ...
        Inimigo *z = &h->pool[i];
        if (!z->vivo || z->estado == ZUM_DEAD || z->atingido_no_ataque) continue;
        if (z->estado == ZUM_HURT) continue;
        if (agora - z->ultimo_dano_recebido < KNOCKBACK_ZUMBI_DURACAO) continue;

        float hzx = z->x + ZUMBI_HBX_OFFSET_X, hzy = z->y + ZUMBI_HBX_OFFSET_Y;
        float hzw = ZUMBI_HBX_W,                hzh = ZUMBI_HBX_H;
        ...
        if (atk_x < hzx+hzw && atk_x+atk_w > hzx && atk_y < hzy+hzh && atk_y+atk_h > hzy)
        {
            ...
            if (z->vida <= 0)
            {
                z->estado = ZUM_DEAD; z->frame = 0;
                if (z->tipo == 1) tocar(sons->morte_velocista);
                else              tocar(sons->morte_zumbi);
                h->total_mortos++;
                san->zumbis_mortos++;
                san->ultimo_abate = al_get_time();
                san->regenerando  = 0;
                atualizar_sanidade(san);
                if (h->total_mortos >= TOTAL_ZUMBIS_FASE) h->fase_concluida = 1;
            }
            else
            {
                if (z->tipo == 1) tocar(sons->dano_sofrido_velocista);
                else              tocar(sons->dano_zumbi);
                if (z->estado != ZUM_HURT)
                {
...
void horda_verificar_dano_jogador(Horda *h, Jogador *jog, VidaStatus *vidas, Sons *sons)
{
    ...
    for (int i = 0; i < MAX_ZUMBIS_TELA; i++)
    {
        ...
        if (acertou)
        {
            ...
            if (z->tipo == 1) tocar(sons->dano_velocista);
            else              tocar(sons->dano);
        ...
        else
        {
            if (!z->ataque_som_tocado && frame_ativo == inicio_dano)
            {
                ...
                if (z->tipo == 1) tocar(sons->ataque_velocista);
                else              tocar(sons->dano_miss);
            }
            z->dano_aplicado = 1;
        }
    }
...
void horda_desenhar(Horda *h, ZumbiSprites *spr)
{
    for (int i = 0; i < MAX_ZUMBIS_TELA; i++)
    {
        ...
        ALLEGRO_COLOR tint = (z->tipo == 1) ? al_map_rgb(255,120,120) : al_map_rgb(255,255,255);
        al_draw_tinted_scaled_bitmap(sheet, tint,
                                     fz*ZUMBI_SRC_W, 0, ZUMBI_SRC_W, ZUMBI_SRC_H,
                                     z->x, z->y, ZUMBI_DRAW_W, ZUMBI_DRAW_H, z->direcao);

        if (z->estado != ZUM_DEAD)
        {
            float bw2   = 58.0f;
            float bx2   = z->x + ZUMBI_DRAW_W/2.0f - bw2/2.0f, by2 = z->y + 4.0f;
...
{
    int lo=0, hi=n-1, mid;
    while (lo <= hi) { mid=(lo+hi)/2; if (v<r[mid]) hi=mid-1; else lo=mid+1; }
    return lo;
}
                   ...
                   ALLEGRO_FONT *fonte, ALLEGRO_FONT *fonte_hud, const char *motivo)
{
    ...
    while (1)
    {
        ...
        if (ev.type == ALLEGRO_EVENT_KEY_DOWN)
        {
            if (ev.keyboard.keycode==ALLEGRO_KEY_LEFT  || ev.keyboard.keycode==ALLEGRO_KEY_A) opcao=0;
            if (ev.keyboard.keycode==ALLEGRO_KEY_RIGHT || ev.keyboard.keycode==ALLEGRO_KEY_D) opcao=1;
            if (ev.keyboard.keycode==ALLEGRO_KEY_ENTER || ev.keyboard.keycode==ALLEGRO_KEY_SPACE)
                return (opcao==0) ? 1 : 0;
            if (ev.keyboard.keycode==ALLEGRO_KEY_ESCAPE) return 0;
        }
        if (ev.type != ALLEGRO_EVENT_TIMER) continue;
...
void desenhar_samurai(Jogador *jog, SamuraiSprites *spr)
{
    ...
    switch (jog->estado)
    {
        case SAM_HURT:
        {
            ...
            if (elapsed >= KNOCKBACK_SAMURAI_DURACAO) { jog->estado=SAM_IDLE; jog->frame_hurt=0; }
            break;
        }
        case SAM_DEAD:
        {
            int f = (int)jog->frame_dead;
            if (f >= FRAMES_DEAD) { f=FRAMES_DEAD-1; jog->morte_animando=0; }
        ...
        case SAM_ATTACK:
        {
            int mf = (jog->tipo_ataque==1)?6:(jog->tipo_ataque==2)?4:3;
            int f  = (int)jog->frame_ataque; if (f>=mf) f=mf-1;
            ALLEGRO_BITMAP *atk = (jog->tipo_ataque==1)?spr->attack1:(jog->tipo_ataque==2)?spr->attack2:spr->attack3;
            ...
            {
                jog->atacando=0; jog->frame_ataque=0; jog->estado=SAM_IDLE;
                jog->som_ataque_tocado=0; jog->dash_som_tocado=0;
            }
            break;
        }
        case SAM_DASH:
...
{
    if (jog->estado==SAM_DEAD || jog->estado==SAM_HURT || jog->estado==SAM_DASH) return;
...
int main(void)
{
    ...
    Horda    horda;
    horda_init(&horda);
    for (int iz = 0; iz < MAX_ZUMBIS_TELA; iz++) horda.pool[iz].ja_stunado_no_dash = 0;
    ...
    while (rodando)
    {
        ...
        if (!pausado && !em_game_over && !horda.fase_concluida)
        {
            ...
            jogador.frame  += 0.15f;
            jogador.movendo = 0;
            if (tempo.ativo) tempo.atual = al_get_time() - tempo.inicio;
                ...
                (agora_atk - jogador.ultimo_ataque) >= delay_atual)
            {
                ...
                if (jogador.estamina >= custo_est)
                {
                    ...
                    if (jogador.tipo_ataque==3) tocar(sons.katana_attack3);
                    else                        tocar(sons.katana12);
        ...
        desenhar_hud_ataque(sam_spr.attack1, sam_spr.attack2, sam_spr.attack3,
                            habilidade1, habilidade2, habilidade3, jogador.tipo_ataque, fonte_hud);
        desenhar_hud_dash_fuga(&jogador, fonte_hud);

        if (pausado)
        {
            al_draw_filled_rectangle(0,0,LARGURA,ALTURA,al_map_rgba(0,0,0,160));
            al_draw_filled_rounded_rectangle(LARGURA/2.0-310,ALTURA/2.0-90,LARGURA/2.0+310,ALTURA/2.0+70,
                                             12,12,al_map_rgb(101,60,20));
            al_draw_rounded_rectangle(LARGURA/2.0-310,ALTURA/2.0-90,LARGURA/2.0+310,ALTURA/2.0+70,
                                      12,12,al_map_rgb(218,165,32),3);
            al_draw_text(fonte,al_map_rgb(255,215,0),LARGURA/2.0,ALTURA/2.0-75,ALLEGRO_ALIGN_CENTER,"PAUSADO");
            al_draw_text(fonte_hud,al_map_rgb(255,255,255),LARGURA/2.0,ALTURA/2.0,ALLEGRO_ALIGN_CENTER,
                         "Pressione ESC para continuar");
        }
        if (roda_aberta)
            desenhar_roda_habilidade(sam_spr.attack1, sam_spr.attack2, sam_spr.attack3,
                                     habilidade1, habilidade2, habilidade3, roda_selecao, fonte_hud);
        ...
        if (morte_pronta || sanidade.game_over)
        {
            ...
            {
                jogador = (Jogador){0};
                jogador.mov.x = 60; jogador.mov.y = 253;
                jogador.tipo_ataque = 1; jogador.estamina = MAX_ESTAMINA;
                jogador.estado = SAM_IDLE; jogador.hurt_inicio = -999.0;
                jogador.ultimo_ataque = -999.0; jogador.dash_fuga_ultimo = -999.0;
                ...
                for (int i = 0; i < MAX_VIDAS; i++)
                    { vetor_vidas[i].ativa=1; strcpy(vetor_vidas[i].status,"Inteira"); }
                tempo = (Temporizador){0};
                tempo.inicio = al_get_time(); tempo.ativo = 1;
                carregar_ranking(&tempo);
        ...
        if (horda.fase_concluida && !sanidade.game_over)
        {
            ...
            al_draw_text(fonte,al_map_rgb(255,215,0),LARGURA/2.0f,ALTURA/2.0f-60,
                         ALLEGRO_ALIGN_CENTER,"50 ZUMBIS ELIMINADOS!");
            al_draw_text(fonte_hud,al_map_rgb(255,255,255),LARGURA/2.0f,ALTURA/2.0f,
                         ALLEGRO_ALIGN_CENTER,"Abrindo ranking...");
            al_flip_display();
            al_rest(2.0);
    ...
    al_destroy_bitmap(sam_spr.idle);   al_destroy_bitmap(sam_spr.run);
    al_destroy_bitmap(sam_spr.jump);   al_destroy_bitmap(sam_spr.attack1);
    al_destroy_bitmap(sam_spr.attack2);al_destroy_bitmap(sam_spr.attack3);
    al_destroy_bitmap(sam_spr.hurt);   al_destroy_bitmap(sam_spr.dead);
    al_destroy_bitmap(zum_spr.walk);   al_destroy_bitmap(zum_spr.run);
    al_destroy_bitmap(zum_spr.attack1);al_destroy_bitmap(zum_spr.attack2);
    al_destroy_bitmap(zum_spr.attack3);al_destroy_bitmap(zum_spr.bite);
    al_destroy_bitmap(zum_spr.hurt);   al_destroy_bitmap(zum_spr.dead);
    al_destroy_bitmap(zum_spr.idle);
    al_destroy_bitmap(coracao);        al_destroy_bitmap(spr_estamina);
    if (spr_sanidade) al_destroy_bitmap(spr_sanidade);
    if (spr_pocao)    al_destroy_bitmap(spr_pocao);
    al_destroy_bitmap(habilidade1);    al_destroy_bitmap(habilidade2);
    al_destroy_bitmap(habilidade3);
    al_destroy_font(fonte_hud);        al_destroy_font(fonte);
